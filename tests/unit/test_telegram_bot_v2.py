...
from app.push.telegram_bot_v2 import TelegramMessageHandler, TelegramSessionStore
...
class TelegramBotV2Tests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_prepare_then_confirm_flow(self) -> None:
        ...
        self.assertIn("偏空结论", confirm_messages[0].text)
        self.assertIn("结论: 卖出", confirm_messages[0].text)
    ...
    def test_confirm_without_pending_task(self) -> None:
        messages = self.handler.handle_update({"message": {"message_id": 3, "chat": {"id": 102}, "text": "确认"}})
        self.assertIn("当前没有待确认任务", messages[0].text)

    def test_run_command_bypasses_prepare(self) -> None:
        messages = self.handler.handle_update({"message": {"message_id": 4, "chat": {"id": 103}, "text": "/run 兴业银行"}})
