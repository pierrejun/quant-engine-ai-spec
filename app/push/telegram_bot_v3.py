...
class TelegramMessageHandler:
    ...
    def handle_update(self, update: dict) -> list[OutboundMessage]:
        ...
        if normalized.startswith("/refresh "):
            return self._run_query(chat_id, message_id, text.split(" ", 1)[1].strip(), True)
        if normalized.startswith("/run "):
            return self._run_query(chat_id, message_id, text.split(" ", 1)[1].strip(), False)
        if normalized.startswith("/report "):
            return self._send_report(chat_id, message_id, text.split(" ", 1)[1].strip(), False)
        if normalized.startswith("/detail "):
            return self._send_report(chat_id, message_id, text.split(" ", 1)[1].strip(), True)
    ...
    def _run_query(self, chat_id: int, message_id: int | None, query: str, force_refresh: bool, as_of: str | None = None, use_demo_data: bool | None = None) -> list[OutboundMessage]:
        ...
        self.session_store.delete(chat_id)
        return [OutboundMessage(chat_id, result["message"], message_id), OutboundMessage(chat_id, result.get("followup_message", ""), message_id)]
