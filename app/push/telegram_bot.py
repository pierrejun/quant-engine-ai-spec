...
class TelegramMessageHandler:
    ...
    def handle_update(self, update: dict) -> list[OutboundMessage]:
        ...
        if normalized.startswith("/refresh "):
            ...
            return self._run_query(chat_id, message_id, query, force_refresh=True)
        if normalized.startswith("/run "):
            query = text.split(" ", 1)[1].strip()
            return self._run_query(chat_id, message_id, query, force_refresh=False)
    ...
    def _prepare(self, chat_id: int, message_id: int | None, query: str) -> list[OutboundMessage]:
        ...
        if prepared["status"] == "unsupported_market":
            return [OutboundMessage(chat_id, prepared["message"], message_id)]
    ...
    def _confirm(self, chat_id: int, message_id: int | None) -> list[OutboundMessage]:
        ...
        return self._run_query(
            ...
            as_of=session["as_of"],
            force_refresh=False,
            use_demo_data=bool(session.get("use_demo_data", self.use_demo_data)),
        )
    ...
    def _build_help_text(self) -> str:
        ...
        return "\n".join(
            [
                ...
                "快捷指令: /run NVDA 直接分析，/refresh NVDA 强制重跑，/cancel 取消当前待确认任务。",
            ]
        )
...
def build_telegram_runtime(project_root: Path) -> TelegramBotRunner:
    ...
    handler = TelegramMessageHandler(
        query_service=QueryService(project_root),
