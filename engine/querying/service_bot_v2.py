...
from engine.querying.formatter_bot_v2 import (
...
class QueryService:
    ...
    def prepare_query(self, query: str, as_of: str | None = None, use_demo_data: bool = False) -> dict:
        ...
        payload["message"] = (
            format_resolution_message(payload["resolution"], analysis_date, payload["cache_hit"])
            if resolution.supported
            else format_unsupported_message(payload["resolution"])
        )
        return payload
        ...
        if prepared["cache_hit"] and not force_refresh:
            ...
            return {
                **prepared,
                ...
                "followup_message": format_followup_message(result),
            }
    ...
    def get_cached_report_text(
        ...
        if cached is None:
            return {
                **prepared,
                ...
                "message": f"{resolution.display_name} 在 {prepared['as_of']} 还没有已生成的报告，请先运行分析。",
            }
        ...
        if not report_path:
            return {
                **prepared,
                "status": "missing_report",
                "message": f"{resolution.display_name} 在 {prepared['as_of']} 的报告路径不存在。",
            }
